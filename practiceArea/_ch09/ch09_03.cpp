...
void interpolation_search(vector<int> &vec, int foundNum);
...
int main()
{
    ...
    cout << "Please enter search mode:"
         << "\n(1) binary search, (2) linear search (3) interpolation_search ";
    cin >> search_mode;
    ...
    case LSR:
        ...
        break;
    case ISR:
        interpolation_search(vec, search_number);
        break;
    default:
        break;
    }
...
void interpolation_search(vector<int> &vec, int foundNum)
{
    ...
    ////mid=low+((val-data[low])*(high-low)/(data[high]-data[low]));
    //int mid = low + ((val - vec[low]) * (high - low) / (vec[high] - vec[low]));
